...
const SingleFieldForm = (field: FormListFieldData, remove?: (index: number | number[]) => void) => {
  return (
    <Space key={field.key}>
    ...
    <Form.List name={['modelConfig', 'models']}>
      {(fields, {add, remove}) => (
        <div style={{display: "flex", rowGap: 16, flexDirection: 'column'}}>
              ...
              >
                {
                  groupKey ? (
                    <Space>
                      ...
                      <Form.Item label="类别" name={[field.name, 'type']}>
                        <Input/>
                      </Form.Item>
                        ...
                        {(subFields, subOpt) => (
                          <div style={{display: "flex", rowGap: 16, flexDirection: 'column'}}>
                            {subFields.map((subField) => (
                              SingleFieldForm(subField, subOpt.remove)
                            ))}
          ...
          <Button type="primary" onClick={() => add({groupName: '分组', groupKey: 'group'})} block>
            添加分组
          </Button>
